...
-- 2.	¿Cuántos pasajeros se movilizaron por cada hora?
CREATE VIEW pasajeros_por_Hora AS
SELECT MAX(TIME(STR_TO_DATE(`Fecha Transaccion`, '%m/%d/%Y %k:%i:'))) as Hora, COUNT(DISTINCT Tarjeta) AS `Pasajeros`
from transacciones
GROUP BY hour(TIME(STR_TO_DATE(`Fecha Transaccion`, '%m/%d/%Y %k:%i:')))
ORDER BY HOUR(TIME(STR_TO_DATE(`Fecha Transaccion`, '%m/%d/%Y %k:%i:')));
...
SELECT MAX(SEC_TO_TIME(TIME_TO_SEC(TIME(STR_TO_DATE(`Fecha Transaccion`, '%m/%d/%Y %k:%i:'))) -
                       TIME_TO_SEC(TIME(STR_TO_DATE(`Fecha Transaccion`, '%m/%d/%Y %k:%i:')) % (25 * 60)))) as Hora,
       round((SUM(`Tarifa-Monto`) / 100), 2)                                                                AS `Ingresos`
from transacciones
GROUP BY SEC_TO_TIME(TIME_TO_SEC(TIME(STR_TO_DATE(`Fecha Transaccion`, '%m/%d/%Y %k:%i:'))) -
                     TIME_TO_SEC(TIME(STR_TO_DATE(`Fecha Transaccion`, '%m/%d/%Y %k:%i:')) % (25 * 60)))
